...
export class RegistryService {
    ...
    getRegistries(titleType: string = null): Observable<Registry[]> {
        ...
        return this.http.get<Registry[]>(this.requestUrl, { params }).pipe(
    ...
    getRegistry(id: string): Observable<Registry> {
        return this.http.get<Registry>(`${this.requestUrl}/${id}`).pipe(
    ...
    getRegistryByCode(code: string): Observable<Registry> {
        return this.http.get<Registry>(`${this.requestUrl}/code/${code}`).pipe(
            map((response) => new Registry(response)),
            catchError(this.handleError),
        );
    }
    ...
    updateRegistry(registry: Registry): Observable<Registry> {
        return this.http.put<Registry>(`${this.requestUrl}/${registry.id}`, registry).pipe(
            map((response) => new Registry(response)),
            catchError(this.handleError),
        );
    }
    ...
    createRegistry(registry: Registry): Observable<Registry> {
        return this.http.post<Registry>(this.requestUrl, registry).pipe(
            map((response) => new Registry(response)),
            catchError(this.handleError),
        );
    }
    ...
    deleteRegion(registry: Registry): Observable<Registry> {
        return this.http.delete<Registry>(`this.requestUrl/${registry.id}`).pipe(
            map((response) => new Registry(response)),
            catchError(this.handleError),
        );
    }
